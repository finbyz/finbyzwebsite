...
const pageData = await fetchFrappeSchemaData({
  ...
  console.log(`${process.env.NEXT_PUBLIC_ERP_URL}/web-api/fb/n/${pageData?.data?.meta_image || pageData?.data?.svg_image || pageData?.data?.image || pageData?.data?.animated_image}`)
...
export default async function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      ...
      <BusinessSlider />
      <StructureData name="web-app-development" type="webpage" />
    </>
  );
}
