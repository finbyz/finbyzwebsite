...
export async function generateMetadata(): Promise<Metadata> {
  const pageData = await fetchFrappeSchemaData({
    name: "forward-contract-in-erpnext",
    type: "webpage"
  })
...
export default async function Layout({ children }: { children: React.ReactNode }) {
  const data = await getPageData("Web Page", "erpnext/apps/forward-contract-erpnext");
  const faqsGroup = await getFaqs("Web Page", "erpnext/apps/forward-contract-erpnext");
