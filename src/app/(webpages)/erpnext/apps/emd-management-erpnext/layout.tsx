...
export async function generateMetadata(): Promise<Metadata> {
  const pageData = await fetchFrappeSchemaData({
    name: "emd-management-in-erpnext",
    type: "webpage"
  })
...
export default async function Layout({ children }: { children: React.ReactNode }) {
  const data = await getPageData("Web Page", "erpnext/apps/emd-management-erpnext");
  const faqsGroup = await getFaqs("Web Page", "erpnext/apps/emd-management-erpnext");
  ...
  return (
    <>
      <main>
        ...
        <BusinessSlider />
        <StructureData name="emd-management-in-erpnext" type="webpage" />
