...
export async function generateMetadata(): Promise<Metadata> {
  const pageData = await fetchFrappeSchemaData({
    name: "rodtep-claim-in-the-erpnext",
    type: "webpage"
  })
...
export default async function Layout({ children }: { children: React.ReactNode }) {
  const data = await getPageData("Web Page", "erpnext/apps/rodtep-claim-erpnext");
  const faqsGroup = await getFaqs("Web Page", "erpnext/apps/rodtep-claim-erpnext");
  ...
  return (
    <>
      <main>
        ...
        <BusinessSlider />
        <StructureData name="rodtep-claim-in-the-erpnext" type="webpage" />
