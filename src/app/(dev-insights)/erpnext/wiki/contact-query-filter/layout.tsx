...
export default async function Layout({ children }: { children: React.ReactNode }) {
  const data = await getPageData("Code Snippet", "contact-query-filter");
  const faqsGroup = await getFaqs("Code Snippet", "contact-query-filter");
  ...
  return (
    <>
      ...
      <StructureData name="SNI-00080" type="code-snippet" />  
      
    <FooterSection doctype="Code Snippet" docname="contact-query-filter" />
    </>
  );
}