...
export default async function Layout({ children }: { children: React.ReactNode }) {
  const data = await getPageData("Code Snippet", "server-side-query");
  const faqsGroup = await getFaqs("Code Snippet", "server-side-query");
  ...
  return (
    <>
      ...
      <StructureData name="SNI-00081" type="code-snippet" />  
      
    <FooterSection doctype="Code Snippet" docname="server-side-query" />
    </>
  );
}