...
export default async function Layout({ children }: { children: React.ReactNode }) {
  const data = await getPageData("Code Snippet", "fetch-table-all-rows");
  const faqsGroup = await getFaqs("Code Snippet", "fetch-table-all-rows");
  ...
  return (
    <>
      ...
      <StructureData name="SNI-00017" type="code-snippet" />  
      
    <FooterSection doctype="Code Snippet" docname="fetch-table-all-rows" />
    </>
  );
}