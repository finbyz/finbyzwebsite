...
export default async function Layout({ children }: { children: React.ReactNode }) {
  const data = await getPageData("Code Snippet", "make-read-only-field");
  const faqsGroup = await getFaqs("Code Snippet", "make-read-only-field");
  ...
  return (
    <>
      ...
      <StructureData name="SNI-00089" type="code-snippet" />  
      
    <FooterSection doctype="Code Snippet" docname="make-read-only-field" />
    </>
  );
}