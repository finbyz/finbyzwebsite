...
export default async function Layout({ children }: { children: React.ReactNode }) {
  const data = await getPageData("Code Snippet", "validate-on-loop");
  const faqsGroup = await getFaqs("Code Snippet", "validate-on-loop");
  ...
  return (
    <>
      ...
      <StructureData name="SNI-00074" type="code-snippet" />  
      
    <FooterSection doctype="Code Snippet" docname="validate-on-loop" />
    </>
  );
}