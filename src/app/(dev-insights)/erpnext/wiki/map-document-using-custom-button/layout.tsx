...
export default async function Layout({ children }: { children: React.ReactNode }) {
  const data = await getPageData("Code Snippet", "map-document-using-custom-button");
  const faqsGroup = await getFaqs("Code Snippet", "map-document-using-custom-button");
  ...
  return (
    <>
      ...
      <StructureData name="SNI-00093" type="code-snippet" />  
      
    <FooterSection doctype="Code Snippet" docname="map-document-using-custom-button" />
    </>
  );
}