...
import {
  MapPin, Briefcase, Building2, Calendar, Clock,
  Search, ChevronRight, BadgeIndianRupee, Users
} from 'lucide-react'
...
function formatSalary(lower: number, upper: number, currency: string, per: string) {
  const fmt = (n: number) =>
    n >= 100000 ? `${(n / 100000).toFixed(1).replace(/\.0$/, '')}L` : n >= 1000 ? `${(n / 1000).toFixed(0)}K` : `${n}`
...
export default function JobOpeningsClient() {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        ...
        else { throw new Error(data.error || 'Failed to fetch jobs') }
  ...
  const filteredJobs = jobs_opening.filter(job => {
    ...
    const matchesSearch = !q || (job.job_title || job.title || '').toLowerCase().includes(q) ||
      (job.small_description || '').toLowerCase().includes(q) || (job.department || '').toLowerCase().includes(q)
  ...
  return (
    <section className="container-custom pt-8 pb-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-3">
          ...
          <div className="flex flex-wrap gap-2">
            {departments.map(dept => (
              <button key={dept} onClick={() => setSelectedCategory(dept)}
        ...
        {!loading && !error && (
          <p className="text-sm text-gray-500 mb-4">{filteredJobs.length} {filteredJobs.length === 1 ? 'opening' : 'openings'} found</p>
        )}
        {loading ? (
          ...
          <div className="grid gap-5 sm:grid-cols-1 lg:grid-cols-1">
            {filteredJobs.map((job, idx) => {
              ...
              const salaryText = job.publish_salary_range ? formatSalary(job.lower_range, job.upper_range, job.currency || 'INR', job.salary_per || 'Month') : null
              const closesDate = formatDate((job as any).closes_on)
              const postedDate = formatDate(job.posted_on)
              return (
                <div key={(job as any).name ?? `${job.job_title}-${idx}`}
                  ...
                  <div className="p-6">
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <h2 className="text-xl font-bold text-[#1A5276] group-hover:text-[#1A5276] leading-tight mb-1">{job.job_title || job.title}</h2>
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 mt-2 text-sm text-gray-500">
                          {job.location && <span className="flex items-center gap-1"><MapPin className="w-3.5 h-3.5 text-gray-400" />{job.location}</span>}
                          {job.employment_type && <span className="flex items-center gap-1"><Briefcase className="w-3.5 h-3.5 text-gray-400" />{job.employment_type.replace(/_/g, ' ')}</span>}
                          {job.department && <span className="flex items-center gap-1"><Building2 className="w-3.5 h-3.5 text-gray-400" />{job.department.replace(/ - FT$/, '')}</span>}
                          {salaryText && <span className="flex items-center gap-1 text-green-600 font-medium"><BadgeIndianRupee className="w-3.5 h-3.5" />{salaryText}</span>}
                        </div>
                        <div className="flex flex-wrap gap-2 mt-3">
                          {job.employment_type && (
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border ${
                              job.employment_type === 'INTERN' ? 'bg-purple-50 border-purple-200 text-purple-700'
                              : job.employment_type === 'Full-time' ? 'bg-blue-50 border-blue-200 text-blue-700'
                              : 'bg-orange-50 border-orange-200 text-orange-700'
                            }`}>{job.employment_type.replace(/_/g, ' ')}</span>
                          )}
                          {job.department && <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-[#1A5276]/8 border border-[#1A5276]/15 text-[#1A5276]">{job.department.replace(/ - FT$/, '')}</span>}
                        </div>
                        {job.small_description && <p className="mt-3 text-gray-600 text-sm leading-relaxed">{job.small_description}</p>}
                      </div>
                      <div className="sm:shrink-0 sm:text-right flex sm:flex-col items-center sm:items-end justify-between gap-3">
                        <div className="space-y-1 text-xs text-gray-400">
                          {postedDate && <div className="flex items-center gap-1 sm:justify-end"><Calendar className="w-3 h-3" /><span>Posted {postedDate}</span></div>}
                          {closesDate && <div className="flex items-center gap-1 sm:justify-end text-red-400"><Clock className="w-3 h-3" /><span>Closes {closesDate}</span></div>}
                        </div>
                      </div>
                    </div>
                    <div className="mt-5 flex flex-col sm:flex-row gap-3 pt-4 border-t border-gray-100">
                      <Link href={`/careers/job-openings/${slug}`}
                      ...
                      <Link href={`/careers/apply?job_title=${encodeURIComponent(job.job_title || job.title || '')}`}
