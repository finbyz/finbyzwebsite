...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      type: "website",
      locale: "en_US",
      images: [{ url: `${process.env.SITE_URL}`, width: 1200, height: 630, alt: pageData?.data?.seo_title }],
    },
    twitter: {
...
export default async function Layout({ children }: { children: React.ReactNode }) {
  const data = await getPageData("Code Snippet", "restrict-users-to-add");
  const faqsGroup = await getFaqs("Code Snippet", "restrict-users-to-add");
