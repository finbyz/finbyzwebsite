...
export default async function Layout({ children }: { children: React.ReactNode }) {
  const data = await getPageData("Code Snippet", "install-python3-x-version-in-ubuntu");
  const faqsGroup = await getFaqs("Code Snippet", "install-python3-x-version-in-ubuntu");
  ...
  return (
    <>
      ...
      <StructureData name="SNI-00477" type="code-snippet" />  
      <BusinessSlider />
    </>
  );
}
