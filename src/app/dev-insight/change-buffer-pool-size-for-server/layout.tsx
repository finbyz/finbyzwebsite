...
export default async function Layout({ children }: { children: React.ReactNode }) {
  const data = await getPageData("Code Snippet", "change-buffer-pool-size-for-server");
  const faqsGroup = await getFaqs("Code Snippet", "change-buffer-pool-size-for-server");
  ...
  return (
    <>
      ...
      <StructureData name="SNI-00470" type="code-snippet" />  
      <BusinessSlider />
    </>
  );
}
