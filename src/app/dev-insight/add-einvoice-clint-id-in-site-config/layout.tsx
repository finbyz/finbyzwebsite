...
export default async function Layout({ children }: { children: React.ReactNode }) {
  const data = await getPageData("Code Snippet", "add-einvoice-clint-id-in-site-config");
  const faqsGroup = await getFaqs("Code Snippet", "add-einvoice-clint-id-in-site-config");
  ...
  return (
    <>
      ...
      <StructureData name="SNI-00464" type="code-snippet" />  
      <BusinessSlider />
    </>
  );
}
