...
export default async function Layout({ children }: { children: React.ReactNode }) {
  const data = await getPageData("Code Snippet", "install-postgres-ubuntu");
  const faqsGroup = await getFaqs("Code Snippet", "install-postgres-ubuntu");
  ...
  return (
    <>
      ...
      <StructureData name="SNI-00475" type="code-snippet" />  
      <BusinessSlider />
    </>
  );
}
