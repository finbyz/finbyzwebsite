...
export default async function Layout({ children }: { children: React.ReactNode }) {
  const data = await getPageData("Code Snippet", "reset-mysql-root-password");
  const faqsGroup = await getFaqs("Code Snippet", "reset-mysql-root-password");
  ...
  return (
    <>
      ...
      <StructureData name="SNI-00469" type="code-snippet" />  
      <BusinessSlider />
    </>
  );
}
