...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      type: "website",
      locale: "en_US",
      images: [{ url: "https:we/", width: 1200, height: 630, alt: pageData?.data?.seo_title }],
    },
    twitter: {
      ...
      creator: "@finbyz",
      images: ["https:we/" + pageData?.data?.title || ""],
    },
    robots: {
      index: true,
      follow: true,
...
export default async function Layout({ children }: { children: React.ReactNode }) {
  const data = await getPageData("Code Snippet", "add-doctype-in-rename-tool");
  const faqsGroup = await getFaqs("Code Snippet", "add-doctype-in-rename-tool");
  ...
  return (
    <>
      ...
      <StructureData name="SNI-00457" type="code-snippet" />  
      <BusinessSlider />
    </>
  );
}
