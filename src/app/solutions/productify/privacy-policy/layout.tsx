...
export async function generateMetadata(): Promise<Metadata> {
  const pageData = await fetchFrappeSchemaData({
    name: "productify-privacy-policy",
    type: "webpage"
  })
...
export default async function Layout({ children }: { children: React.ReactNode }) {
  const data = await getPageData("Web Page", "productify-privacy-policy");
  const faqsGroup = await getFaqs("Web Page", "productify-privacy-policy");
  ...
  return (
    <>
      <main>
        ...
        <BusinessSlider />
        <StructureData name="productify-privacy-policy" type="webpage" />
        <InquiryForm />
      </main>
