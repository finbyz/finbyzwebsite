...
const pageData = await fetchFrappeSchemaData({
    name: "erp-implementation-phases",
    type: "blog"
})
return {
  ...
  openGraph: {
    ...
    type: "website",
    locale: "en_US",
    images: [{ url: `${process.env.NEXT_PUBLIC_ERP_URL}/web-api/fb/n/${pageData?.data?.meta_image || pageData?.data?.svg_image || pageData?.data?.image || pageData?.data?.animated_image}` || `${process.env.SITE_URL}/images/Phases_of_ERP_Implementation.svg`, width: 1200, height: 630, alt: pageData?.data?.seo_title }],
  },
  twitter: {
...
export default async function Layout({ children }: { children: React.ReactNode }) {
  const faqsGroup = await getFaqs("Blog Post", "erp/insights/erp-implementation-phases");
  const data = await getPageData("Blog Post", "erp/insights/erp-implementation-phases");
