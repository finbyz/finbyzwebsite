...
const pageData = await fetchFrappeSchemaData({
    name: "cost-benefit-analysis-of-erp-investments-in-the-chemical-sector",
    type: "blog"
})
return {
  ...
  openGraph: {
    ...
    type: "website",
    locale: "en_US",
    images: [{ url: `${process.env.NEXT_PUBLIC_ERP_URL}/web-api/fb/n/${pageData?.data?.meta_image || pageData?.data?.svg_image || pageData?.data?.image || pageData?.data?.animated_image}` || `${process.env.SITE_URL}/images/15-04-24.svg`, width: 1200, height: 630, alt: pageData?.data?.seo_title }],
  },
  twitter: {
...
export default async function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      ...
      <LazyBusinessSlider />
      <StructureData name="cost-benefit-analysis-of-erp-investments-in-the-chemical-sector" type="blog" />
    </>
  );
}
