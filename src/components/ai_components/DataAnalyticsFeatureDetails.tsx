...
export default function DataAnalyticsFeatureDetails({ details }: { details: { title: string; description: string; src: string }[] }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
      {details.map((item) => (
        <div key={item.title} className="flex flex-col items-center bg-white/80 rounded-lg p-6 shadow-md hover:shadow-lg transition-shadow duration-300">
          ...
          <p className="text-justify text-muted-foreground text-base">{item.description}</p>
        </div>
      ))}
    </div>
  );
